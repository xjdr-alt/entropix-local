...
#global imports
from entropix.local.config import MODEL_ID, MODEL_PATH
...
def download_weights(model_id: str = MODEL_ID, out_dir: Path = Path(MODEL_PATH)):
    ...
    with patch("transformers.dynamic_module_utils.get_imports", fixed_get_imports):
        hf_model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.bfloat16, offload_folder="/tmp/offload")
        with torch.no_grad():
